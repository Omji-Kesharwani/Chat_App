...
const ProfilePage = () => {
  const {user,isAuth,loading,setUser} 
  =useAppData();
  ...
  const submitHandler = async(e:any)=>{
   ...
   try{
    const {data}=await axios.post(`${user_service}/api/v1/user/update-name`,{name},{
      headers:{
      ...
      <div className="max-w-2xl mx-auto pt-8">
        <div className="flex items-center gap-4  mb-8">
          <button onClick={()=>router.push("/chat")} className='p-3 bg-gray-800 hover:bg-gray-700 rounded-lg border border-gray-700'>
            ...
            <div>
              <h1 className="text-3xl font-bold text-white">Profile Settinngs</h1>
              <p className="text-gray-400 mt-1 ">Manage your account information
              </p>
            </div>
        </div>
              ...
              <div>
                <label className='block text-sm font-semibold text-gray-300 mb-3 '>Display Name</label>
                {
                  isEdit ?
                   ...
                   <form onSubmit={submitHandler} className='space-y-4'>
                    <div className='relative'>
                      ...
                      className='w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white placholder-gray-400'/>
                    ...
                    <div className="flex gap-3">
                      <button type='submit' className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg">
                        ...
                        <button 
                        type="button"
                        onClick={editHandler} className='flex items-center gap-2 px-6 py-3 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-lg'>
                          Cancel
                        </button>
                    </div>
                  </form>
                  ...
                  <div className='flex items-center justify-between p-4 bg-gray-700 rounded-lg border border-gray-600'>
                    ...
                    className='flex items-center gap-2 px-6 py-3 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-lg'>Edit</button>
                  </div>
                )
                }
              </div>
