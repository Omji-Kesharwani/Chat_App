...
const MessageInput = ({
  ...
  message,
  setMessage,
  handleMessageSend}:MessageInputProps) => {
    ...
    return (
      <form onSubmit={handleSubmit} className="flex flex-col gap-2 border-t border-gray-700 pt-2">
       ...
       <div className='flex items-center gap-2'>
        ...
        <button type='submit' disabled={(!imageFile && !message) || isUploading} className='bg-blue-600 hover:bg-blue-700 p-4 py-2 rounded-lg transition-colors flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed'>
          {
            isUploading ? <Loader2 classNmae="w-4 h-4 animate-spin"/>:<Send className="w-4 h-4 "/>
          }
        </button>
       </div>
      </form>
